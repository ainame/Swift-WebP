...
public struct WebPDecoderConfig: InternalRawRepresentable {
    ...
    public static func initialized() -> WebPDecoderConfig {
        var originConfig = CWebP.WebPDecoderConfig()
        if (CWebP.WebPInitDecoderConfig(&originConfig) != Int32(VP8_STATUS_OK.rawValue)) {
            fatalError("can't init decoder config")
        }
        return WebPDecoderConfig(rawValue: originConfig)!
    }
    ...
    public enum Colorspace {
        ...
        var RGBA: UnsafeMutablePointer<UInt8> {
            if case .RGBA(let buffer) = self  {
                return CWebP.WebPDecBuffer.__Unnamed_union_u(RGBA: buffer).RGBA.rgba
            }
            fatalError("please use yuva")
        }
        
        var YUVA: UnsafeMutablePointer<UInt8> {
            if case .YUVA(let buffer) = self {
                return CWebP.WebPDecBuffer.__Unnamed_union_u(YUVA: buffer).YUVA.y
            }
